...
namespace Solution.Base.Controllers.Admin
{
    ...
    public class AdminFileManagerController : BaseControllerAuthorize
    {
        ...
        protected Dictionary<string, string> ParseJSON(string file)
        {
            ...
            try
            {
                json = System.IO.File.ReadAllText(Server.MapPath(file), System.Text.Encoding.UTF8);
            }
            catch (Exception ex) { }
        ...
        protected string GetFilesRoot()
        {
            ...
            if (ret == "")
                ret = Server.MapPath("~/FileManager/Uploads");
            else
                ret = FixPath(ret);
            return ret;
        }
        ...
        protected void VerifyAction(string action)
        {
            ...
            if (Server.MapPath(setting) != Server.MapPath(ControllerContext.HttpContext.Request.Url.LocalPath))
        ...
        public ActionResult DownloadDir(string path)
        {
            ...
            string tmpZip = Server.MapPath("~/FileManager/tmp/" + dirName + ".zip");
        ...
        {
            return (ControllerContext.HttpContext.Request["method"] != null && ControllerContext.HttpContext.Request["method"].ToString() == "ajax");
        }
        protected ActionResult Upload(string path)
        {
            ...
            try
            {
                for (int i = 0; i < ControllerContext.HttpContext.Request.Files.Count; i++)
                {
                    if (CanHandleFile(ControllerContext.HttpContext.Request.Files[i].FileName))
                    {
                        FileInfo f = new FileInfo(ControllerContext.HttpContext.Request.Files[i].FileName);
                        string filename = MakeUniqueFilename(path, f.Name);
                        string dest = Path.Combine(path, filename);
                        ControllerContext.HttpContext.Request.Files[i].SaveAs(dest);
