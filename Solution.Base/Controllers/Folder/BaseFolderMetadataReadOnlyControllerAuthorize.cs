...
namespace Solution.Base.Controllers
{
    ...
    {   
        ...
        public BaseFolderMetadataReadOnlyControllerAuthorize(string physicalPath, Boolean includeSubDirectories, Boolean admin, IFileSystemRepositoryFactory fileSystemRepositoryFactory, IMapper mapper = null, ILogFactory logFactory = null, IEmailService emailService = null)
        ...
        // GET: Default
        [Route("")]
        public virtual async Task<ActionResult> Index(int page = 1, int pageSize = 10, string orderColumn = nameof(DirectoryInfo.LastWriteTime), string orderType = OrderByType.Descending)
        {
        ...
        [ChildActionOnly]
        public virtual ActionResult ViewAllChild(int page = 1, int pageSize = 10, string orderColumn = nameof(DirectoryInfo.LastWriteTime), string orderType = OrderByType.Descending)
        { 
            var cts = TaskHelper.CreateChildCancellationTokenSource(HttpContext.Response.ClientDisconnectedToken);
        
            try
            {
                IEnumerable<DirectoryInfo> data = null;
                int total = 0;
