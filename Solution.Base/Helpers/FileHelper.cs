...
namespace Solution.Base.Helpers
{
    ...
    {
        public static Boolean SaveToDirectory(this HttpPostedFileBase fileUpload, string physicalFolder)
        {
            if (fileUpload != null && fileUpload.ContentLength > 0)
            {
                var fileName = Path.GetFileName(fileUpload.FileName);
                fileUpload.SaveAs(Path.Combine(physicalFolder, fileName));
                return true;
            }
            return false;
        }

        public static string GetFolderPhysicalPathById(this HttpServerUtility server, string folderId)
        {
            return server.MapPath(System.Configuration.ConfigurationManager.AppSettings["Folder:" + folderId]);
        }

        public static string GetFolderPhysicalPathById(this HttpServerUtilityBase server, string folderId)
        {
            return server.MapPath(System.Configuration.ConfigurationManager.AppSettings["Folder:" + folderId]);
        }

        public static bool FileExists(this HttpServerUtilityBase server, string path)
        {
        ...
        public static FileInfo GetFileInfo(this HttpServerUtilityBase server, string path)
        {
            if (FileExists(server, path))
            {
        ...
        public static DirectoryInfo GetDirectoryInfo(this HttpServerUtilityBase server, string folderId, string path)
        {
            return server.GetDirectoryInfo(server.GetFolderPhysicalPathById(folderId) + path);
        }

        public static DirectoryInfo GetDirectoryInfo(this HttpServerUtilityBase server, string path)
        {
            if (FolderExists(server, path))
            {
        ...
        public static string GetAbsoluteUrlSlug(this HttpServerUtilityBase server, string folderId, string path, int width = 0, int height = 0, int size = 0, int maxWidth = 0, int maxHeight = 0, bool watermark = false)
        {
            return AbsoluteUrlSlug(GetFileInfo(server, folderId, path), width, height, size, maxWidth, maxHeight, watermark);
        }
        ...
        {
            string appRelativeVirtualPath = "~/" + physicalPath.Replace(HostingEnvironment.ApplicationPhysicalPath, String.Empty).Replace("\\", "/");
            var absolute = VirtualPathUtility.ToAbsolute(appRelativeVirtualPath);
    ...
    public static class FileHelper
    {
        ...
        {
            byte[] result = null;
            using (System.IO.BinaryReader binaryReader = new System.IO.BinaryReader(file.InputStream))
            {
                result = binaryReader.ReadBytes(file.ContentLength);
            }
            return result;
        }
